...
# Rscript 01.11gtexclump0.2.r 2>&1 | tee 01.11gtexclump0.2.r.log

# ==============================================================================
# 配置区 
# ==============================================================================
WORK_DIR       <- "/home/publicdata/32445978_bookliu11/gtex"
INPUT_FILE     <- "GTEx_V10_MR_Formatted_hg19.qs"
OUTPUT_FILE    <- "GTEx_r2_0.2.qs"
...
# 定义原始目标路径
target_path <- paste0(WORK_DIR, "/", OUTPUT_FILE)
tryCatch({
  # 1. 尝试保存到原始工作目录
  qs::qsave(dt_final, target_path)
  log_msg(paste0("保存成功: ", target_path))
  
}, error = function(e) {
  # 2. 如果报错（如权限不足），捕获错误并切换到 Home 目录
  # path.expand("~") 能自动获取当前用户的家目录，例如 /home/piperacillin
  fallback_path <- file.path(path.expand("~"), OUTPUT_FILE)
  
  log_msg(paste0("警告: 写入 ", target_path, " 失败。"))
  log_msg(paste0("系统报错: ", e$message))
  log_msg(paste0("策略切换: 正在尝试保存到用户主目录 -> ", fallback_path))
  
  # 尝试保存到备用路径
  qs::qsave(dt_final, fallback_path)
  log_msg(paste0("保存成功 (备用路径): ", fallback_path))
})
log_msg("任务完成。")
